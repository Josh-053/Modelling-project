...
"  COM(6)=BW*ABS(G(2,1))
"  COM(7)=ALB*ABS(G(2,1))
"  COM(8)=CREAT*ABS(G(2,1))
$OMEGA  0.573074  ;     IIV CL
 0.2141  ;      IIV V
$SIGMA  0.290424  ; EPS(1), proportional
 5E-13  FIX  ;     EPS(2)  ; additive, required by M7+ censoring method
    ; residual variability
$ESTIMATION METHOD=1 INTERACTION SIGL=4 PRINT=5 CTYPE=4 MAXEVALS=0
$TABLE      MDV ID COM(1)=CLRATIO COM(2)=CLBWNUM COM(3)=CLALBNUM
            COM(4)=CLCREATNUM DV CIPREDI CIWRESI NOAPPEND NOPRINT
            ONEHEADER FORMAT=s1PE23.16 FILE=CL_time_varying.dta
$TABLE      MDV ID COM(5)=V1RATIO COM(6)=V1BWNUM COM(7)=V1ALBNUM
            COM(8)=V1CREATNUM DV CIPREDI CIWRESI NOAPPEND NOPRINT
