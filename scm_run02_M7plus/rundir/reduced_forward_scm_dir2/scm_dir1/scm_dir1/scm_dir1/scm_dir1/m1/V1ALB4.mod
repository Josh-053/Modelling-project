...
;;; V1ALB-DEFINITION START
   V1ALB = EXP(THETA(8)*(ALB - 31.85))
;;; V1ALB-DEFINITION END


;;; V1BW-DEFINITION START
IF(BW.LE.65.93) V1BW = ( 1 + THETA(6)*(BW - 65.93))
IF(BW.GT.65.93) V1BW = ( 1 + THETA(7)*(BW - 65.93))
;;; V1BW-DEFINITION END
...
$THETA  (0.001,1.41628,8) ; [1]   CL (L/d)
 (0.001,7.3696,10) ; [2,3] V1  (L)
 (0.01,0.196283,6) ; V2
 (0.001,1.3775,6) ; Q
 ; values are determined in 3 iterations
$THETA  (-1,4.7557,5) ; CLADA1
$THETA  (-100000,0.0138211,0.042) ; V1BW1
 (-0.055,-0.0509789,100000) ; V1BW2
$THETA  (-0.388950184627373,0.0120993,0.388950184627373) ; V1ALB1
...
$OMEGA  0.601222  ;     IIV CL
 0.376969  ;      IIV V
$SIGMA  0.280131  ; EPS(1), proportional
