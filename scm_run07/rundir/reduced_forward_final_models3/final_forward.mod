...
$INPUT      DUMMY=DROP ID TIME WEEK DOSE=AMT RATE CONC=DROP ADA BW
            ALB=DROP SEX=DROP AGE=DROP CREAT=DROP COL EVID BLQ
            CONC_M7=DV
...
IF(BW.LE.65.97) V1BW = ( 1 + THETA(6)*(BW - 65.97))
IF(BW.GT.65.97) V1BW = ( 1 + THETA(7)*(BW - 65.97))
;;; V1BW-DEFINITION END
...
$THETA  (0.001,0.930744,10) ; [1]   CL (L/d)
 (0.001,2.21826,10) ; [2,3] V1  (L)
 (0.01,0.0399111,10) ; V2
 (0.001,1.35012,10) ; Q
 ; values are determined in 3 iterations
$THETA  (-1,-0.835303,5) ; V1ADA1
$THETA  (-100000,0.00182473,0.042) ; V1BW1
 (-0.055,-0.0274182,100000) ; V1BW2
...
$OMEGA  0.507941  ;     IIV V1
;0.573 ; IIV CL
$SIGMA  1.33435  ; EPS(1), proportional
