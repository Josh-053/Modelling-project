...
!                        ICALL  - 01 = input data file modification (PRED_IGNORE_DATA_TEST=1)
...
!               STR,STR1,TLIST,TRUNTH,MAXLEN,MQQ,MLINE,SPACE
!
!---------------------------- END OF HEADER -----------------------------------------
!
      SUBROUTINE PREDI(ICALL,NEWIND,THETA,INDXS,F,H,IDEF)
!
      USE PRSIZES,      ONLY: DPSIZE,ISIZE,PAL,PC,PCT,PD,PG,PIR,MAXIC,SD
      USE VERSION,      ONLY: LEV
      USE NMDATA,       ONLY: IUN
      USE PRDATA,       ONLY: TEXT,LETT,DASH,STAR,TDIGIT,CNOYES,COFFON,OLDFMT
! INTEGER
      USE NMBAYES_INT,  ONLY: NNATTRIB
      USE NM_BAYES_INT, ONLY: NM_STEP,BASE_STEP
      USE NMBAYES_CHAR, ONLY: ATTRIB_NAME,ATTRIB_VALUE
      USE NMPRD_INT,    ONLY: IERPRD,IPRNV,IPROB,IPS,IQUIT,IRECRSIV,ISETNTH,NITSUP,  &
                              NOEPS,NTHETA,NWEPS,NWETA,NWIND,NWTHT,NETAZ,NEPSZ,PRED_IGNORE_DATA_TEST
      USE PRCM_INT,     ONLY: AA,IAI,IAJ,IAK,INRD,IANRD,IPI,IPJ,IPK,ITI,ITK,ITOTL,LNCM1,   &
      ...
      use PNM_CONFIG,   ONLY: PNM_RUN_MODE,PNM_WORKER
! REAL
      ...
      USE PROCM_REAL,   ONLY: AMNT,D2AETA,D2DOST,DAETA,DDOST,DOSREC,DOSTIM,EVTREC,IREVP
! CHARACTER
...
!
      LOGICAL :: ISIDEF
!
      IF(PRED_IGNORE_DATA_TEST==0.OR.ICALL/=-1) THEN
        TDIGIT(1) = '  *'
        SPACE=' '
        DO I=1,999
...
!      
      SELECT CASE (K)
      CASE (0,1)     ! ICALL=0  Model initialization
...
!
          TSTART=ZERO; ETA=ZERO;  DDOST=ZERO;  COMPAC=.TRUE.; MMAX=PALZ; IREV=IREVP
...
!
        ...
        I_SS=-1
!        
        IF(PNM_RUN_MODE/=PNM_WORKER) NM_STEP=BASE_STEP
        CALL PREDI_SETTOL() !nonmem7.4
        IF(NNATTRIB>0) CALL XML_ATTRIB_ADDI(NNATTRIB,ATTRIB_NAME,ATTRIB_VALUE,'PRED_ADVAN',ADVID)
...
! Allow no basic params with DES
          IF ((NBP <= 0 .OR. NBP >= IRGG) .AND. (NBP /= 0 .OR.      &
              (ADVID /= 6 .AND. ADVID /= 8 .AND. ADVID /= 9 .AND. &
              ADVID /= 13.AND.ADVID/=14 .AND. ADVID/=15 .AND. ADVID /=16 &
                .AND. ADVID/=17.AND. ADVID/=18))) THEN   ! ajb 3/2009              
            IF (NBP <= 0) THEN
...
! Check for error in compartment definitions; Allow no dose compartment with ADVAN9
          IF ((IDC <= 0 .OR. IDC > NCM1) .AND. (IDC /= 0 .OR. (ADVID /= 9 .AND. ADVID/=15 .AND. ADVID/=17) )) THEN
...
!        
        ...
        ELSE
          MQQ=SD
        ENDIF
        IF ( (ADVID /= 9 .AND. ADVID /=15 .AND. ADVID/=17) .OR. .NOT. OLDFMT) THEN
          IF(MQQ<=8) THEN 
            WRITE (LOGUNT,2100,ERR=502)
          ELSE
            WRITE (LOGUNT,2103,ERR=502)
          ENDIF
        ELSE
          IF(MQQ<=8) THEN
            WRITE (LOGUNT,2101,ERR=502)
          ELSE
            WRITE (LOGUNT,2104,ERR=502)
          ENDIF
        END IF
!        
        NBRON=0;   COTHER(1)=' '
...
!        
        DO I=1,NC
          ...
          JDO=1
          IF (I == IDO) JDO=2
          IF (ADVID == 9 .OR. ADVID==15 .OR. ADVID==17) THEN
          ...
          IF (NAMEFL == 1 .OR. I == NC) THEN ! Always use name array for output compartment
            WRITE (LINE,219) I,NAME(I)(1:MQQ),COFFON(ISV(I)+1),CNOYES(ITURN(I)+1),            &
                             CNOYES(IINST(I)+1),CNOYES(JD),CNOYES(JDO),COTHER(1),COTHER(2)
          ELSE
            WRITE (LINE,220) I,(IATT(I,J),J=6,7),SPACE(1:MQQ-3),COFFON(ISV(I)+1),CNOYES(ITURN(I)+1),  &
                             CNOYES(IINST(I)+1),CNOYES(JD),CNOYES(JDO),COTHER(1),COTHER(2)
          END IF
          IF ( (ADVID == 9 .OR. ADVID==15 .OR. ADVID==17) .AND. OLDFMT) THEN
            MLINE=MQQ+88
          ELSE
            MLINE=MQQ+66
          END IF
          WRITE (LOGUNT,'(A)',ERR=502) LINE(1:MLINE)
        END DO
!        
        IF ( (ADVID == 9 .OR. ADVID==15 .OR. ADVID==17) .AND. .NOT. OLDFMT) THEN
          IF(MQQ<=8) THEN
            WRITE (LOGUNT,2102,ERR=502)
          ELSE
            WRITE (LOGUNT,2105,ERR=502)
          ENDIF
          DO I=1,NC
            COTHER(2)=CNOYES(ITOTL(I)+1)
            COTHER(1)=CNOYES(1)
            IF (I <= MCOMP) COTHER(1)=CNOYES(IATT(I,8)+1)
            IF (NAMEFL == 1 .OR. I == NC) THEN
              WRITE (LINE,219) I,NAME(I)(1:MQQ),COTHER(1),COTHER(2)
            ELSE
              WRITE (LINE,220) I,(IATT(I,J),J=6,7),SPACE(1:MQQ-3),COTHER(1),COTHER(2)
            END IF
            MLINE=MQQ+33
            WRITE (LOGUNT,'(A)',ERR=502) LINE(1:MLINE)
          END DO
        END IF
! Special checks for ADVAN9
        IF ( (ADVID == 9 .OR. ADVID==15 .OR. ADVID==17) .AND. LNCM2 > 0) THEN 
          DO I=LNCM1+1,NCM1
            IF (IINST(I) == 1) THEN
...
! 
        IF ((SSID /= 0 .AND. SSID /= 6 .AND. SSID /= ADVID) .AND.  &
            (SSID /= 5 .OR. (ADVID /= 6 .AND. ADVID /= 8 .AND. ADVID /= 13 &
              .AND. ADVID /= 14 .AND. ADVID/=16 .AND. ADVID/=18 )) .AND.   &
            (SSID /= 9 .OR. ( ADVID /=15.AND.  ADVID /=17 )) .AND.   &
            (SSID /= 7 .OR. (ADVID /= 6 .AND. ADVID /= 8 .AND. ADVID /= 13 .AND.&
               ADVID /= 14 .AND. ADVID/=16 .AND. ADVID/=18))) THEN       
...
!
        ...
        CALL WRITETOL('INITIAL (BASE) TOLERANCE SETTINGS:',INRD,IANRD)
!        
        GO TO 999
      CASE (2)
        GO TO 1000
...
! ICALL=1  Problem initialization
 1000 CONTINUE
! IRECRSIV value should be under control of manager.  See pnm_global_rw.f90.  nm76b2, RJB 9/2024
      IF(PNM_RUN_MODE/=PNM_WORKER) IRECRSIV=1 ! Tell NONMEM "recursive" PRED  9/97
                 ! Set up information obtained from NONMEM
...
!      
      IF (PF) THEN
        ...
        IF(NNATTRIB>0) THEN !nonmem7.4
          IF(JEVENT/=JDEF) CALL XML_ATTRIB_ADDI(NNATTRIB,ATTRIB_NAME,ATTRIB_VALUE,'PRED_EVID',JEVENT)
          IF(JTIME/=JDEF) CALL XML_ATTRIB_ADDI(NNATTRIB,ATTRIB_NAME,ATTRIB_VALUE,'PRED_TIME',JTIME)
          IF(JAMT/=JDEF) CALL XML_ATTRIB_ADDI(NNATTRIB,ATTRIB_NAME,ATTRIB_VALUE,'PRED_AMT',JAMT)
          IF(JRATE/=JDEF) CALL XML_ATTRIB_ADDI(NNATTRIB,ATTRIB_NAME,ATTRIB_VALUE,'PRED_RATE',JRATE)
          IF(JSS/=JDEF) CALL XML_ATTRIB_ADDI(NNATTRIB,ATTRIB_NAME,ATTRIB_VALUE,'PRED_SS',JSS)
          IF(JDELTA/=JDEF) CALL XML_ATTRIB_ADDI(NNATTRIB,ATTRIB_NAME,ATTRIB_VALUE,'PRED_II',JDELTA)
          IF(JMORE/=JDEF) CALL XML_ATTRIB_ADDI(NNATTRIB,ATTRIB_NAME,ATTRIB_VALUE,'PRED_ADDL',JMORE)
          IF(JCOMPT/=JDEF) CALL XML_ATTRIB_ADDI(NNATTRIB,ATTRIB_NAME,ATTRIB_VALUE,'PRED_CMT',JCOMPT)
          IF(JCOMPF/=JDEF) CALL XML_ATTRIB_ADDI(NNATTRIB,ATTRIB_NAME,ATTRIB_VALUE,'PRED_PCMT',JCOMPF)
          IF(JERROR/=JDEF) CALL XML_ATTRIB_ADDI(NNATTRIB,ATTRIB_NAME,ATTRIB_VALUE,'PRED_CALL',JERROR)
          IF(JCONT/=JDEF) CALL XML_ATTRIB_ADDI(NNATTRIB,ATTRIB_NAME,ATTRIB_VALUE,'PRED_CONT',JCONT)
        ENDIF
...
!
      IF (JTIME == JDEF) THEN
        IF ( (ADVID /= 9 .AND. ADVID/=15 .AND. ADVID/=17) ) THEN
...
!
      IF (ADVID == 6 .OR. ADVID == 8 .OR. ADVID == 9 .OR. ADVID == 13 .OR. ADVID==14 &
         .OR. ADVID==15 .OR. ADVID==16 .OR. ADVID==17 .OR. ADVID==18) THEN       ! ajb 3/2009      
...
! New library or generated subroutine sets IDEFA(2) = -1 (every event) or IDEFA(2)=1 (once per IR)
        ...
        IF (JTIME == JDEF) THEN   ! If no time data item ...
          ...
          IF (PF) THEN
            ...
            ELSE IF (ADVID==15) THEN
            IF (CALLA9 == -1) WRITE (LOGUNT,3486,ERR=502)
            IF (CALLA9 == 1)  WRITE (LOGUNT,3487,ERR=502)    
            ELSE IF (ADVID==17) THEN
            IF (CALLA9 == -1) WRITE (LOGUNT,3488,ERR=502)
            IF (CALLA9 == 1)  WRITE (LOGUNT,3489,ERR=502)    
            ENDIF
          END IF
...
!    
  ...
  219 FORMAT (2X,I3,9X,A,5X,7(A3,8X))
  220 FORMAT (2X,I3,9X,2A4,A,7(A3,8X))
 ...
 1457 FORMAT ('0FIRST MODEL TIME PARAMETER ASSIGNED TO ROW NO.:',I3, / &
              ' LAST  MODEL TIME PARAMETER ASSIGNED TO ROW NO.:',I3)
 ...
 2100 FORMAT ('0COMPARTMENT ATTRIBUTES '/ &
              ' COMPT. NO.   FUNCTION   INITIAL    ON/OFF      DOSE      DEFAULT    DEFAULT'/ &
              '                         STATUS     ALLOWED    ALLOWED    FOR DOSE   FOR OBS.')
 2101 FORMAT ('0COMPARTMENT ATTRIBUTES '/ &
      ' COMPT. NO.   FUNCTION   INITIAL    ON/OFF      DOSE      DEFAULT    DEFAULT    EQUILIB    EXCLUDE'/ &
      '                         STATUS     ALLOWED    ALLOWED    FOR DOSE   FOR OBS.    RIUM      FROM TOTAL')
 2102 FORMAT ('0COMPT. NO.   FUNCTION   EQUILIB    EXCLUDE'/&
               '                          RIUM      FROM TOTAL')
 2103 FORMAT ('0COMPARTMENT ATTRIBUTES '/ &
      ' COMPT. NO.   FUNCTION                         INITIAL    ', &
      'ON/OFF      DOSE      DEFAULT    DEFAULT'/ &
      '                                               STATUS     ', &
      'ALLOWED    ALLOWED    FOR DOSE   FOR OBS.')
 2104 FORMAT ('0COMPARTMENT ATTRIBUTES '/ &
      ' COMPT. NO.   FUNCTION                         INITIAL    ', &
      'ON/OFF      DOSE      DEFAULT    DEFAULT    EQUILIB    EXCLUDE'/&
      '                                               STATUS     ', &
      'ALLOWED    ALLOWED    FOR DOSE   FOR OBS.    RIUM      FROM TOTAL')
 2105 FORMAT ('0COMPT. NO.   FUNCTION                         EQUILIB    EXCLUDE'/&
              '                                                RIUM      FROM TOTAL')
 ...
 3486 FORMAT ('0ADVAN15 CALLED WITH EVERY EVENT RECORD.')
 3487 FORMAT ('0ADVAN15 CALLED ONCE PER INDIVIDUAL RECORD.')
 3488 FORMAT ('0ADVAN17 CALLED WITH EVERY EVENT RECORD.')
 3489 FORMAT ('0ADVAN17 CALLED ONCE PER INDIVIDUAL RECORD.')
 ...
 4993 FORMAT (' PK MAY NOT MODEL COMPARTMENT ',I3,'''S ',A/ &
              ' BECAUSE DOSES TO THIS COMPARTMENT ARE NOT ALLOWED.')
...
!-------------------------------- PREDI_SETTOL.F90 ----------------------------------
!
! SUBROUTINE PREDI_SETTOL()
!
! DESCRIPTION : Sets Tolerances conditions.
!
!
! ARGUMENTS   : NONE
!               IN     - NONE
...
!               IN OUT - NONE
!
! CALLED BY   : PRED,PREDI
!
! CALLS       : SADVAN,SSS
!
! ALGORITHM   : see description
!
! MODULES USED: PRSIZES,SIZES,PRCOM_INT,PRCM_INT,NM_BAYES_INT,PRMOD_INT
!
! CONTAINS    : NONE
!
! LOCALS      : ICALL,ICALLDP,ICALLP
!
!---------------------------- END OF HEADER -----------------------------------------
!
      SUBROUTINE PREDI_SETTOL() !nonmem7.4
!
      USE PRSIZES,      ONLY: PC
!
...
!
! INTEGER
      USE PRCOM_INT,    ONLY: ADVID,IDNO,NCM1,MCOMP,SSID
      USE PRCM_INT,     ONLY: INRD,IANRD
      USE NM_BAYES_INT, ONLY: NMADVID,NMNCM1,NMSSID,TOLV,ATOLV,ADVID2,SSID2,         &
                              TOLTOPRED
      USE PRMOD_INT,    ONLY: ICALLD
!
      IMPLICIT NONE
!
!
...
! Local Variables
!
      INTEGER(KIND=ISIZE) :: ICALL=0,ICALLDP,ICALLP
!
      IF(TOLTOPRED==1) THEN
        IF(ADVID/=6.AND.ADVID/=8.AND.ADVID/=9.AND.ADVID/=10.AND.ADVID/=13.AND.ADVID/=14.AND.ADVID/=15 &
          .AND. ADVID/=16 .AND. ADVID/=17 .AND. ADVID/=18) GO TO 999
      ENDIF
      ICALLP=ICALL
      ICALLDP=ICALLD
      ICALL=0
      ...
      ICALL=ICALLP
      IF(ADVID/=6.AND.ADVID/=8.AND.ADVID/=9.AND.ADVID/=10.AND.ADVID/=13.AND.ADVID/=14.AND.ADVID/=15 &
      .AND. ADVID/=16 .AND. ADVID/=17 .AND. ADVID/=18) THEN
        IANRD(0:MCOMP)=-100
        INRD(0:MCOMP)=-100
      ENDIF

      ATOLV(0:MCOMP)=IANRD(0:MCOMP)
      TOLV(0:MCOMP)=INRD(0:MCOMP)
      IF(TOLTOPRED/=1) THEN
        NMADVID=ADVID
        NMNCM1=NCM1
        NMSSID=SSID
