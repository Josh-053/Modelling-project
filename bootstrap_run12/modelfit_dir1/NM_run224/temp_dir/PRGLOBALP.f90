...
!
        INTEGER(KIND=ISIZE) :: LST,LEND,SLEND(PE),SLST(PE),SPW,SLOOP,XLNCM1,XLNCM2,LNCM1,LNCM2,      &
                               ...
                               ITOTL(PC),INRD(0:PM),NIAI,NIPI,NIT,XNIAI,XNIPI,XNIT,IAI(PIR),IAJ(PIR),  &
                               ...
                               AA(PIR),PP(PIR),TT(PIR),XAA(PIR),XPP(PIR),XTT(PIR),IANRD(0:PM),MFLAGDDE
!
      END MODULE PRCM_INT      
!
!+++
...
!
        INTEGER(KIND=ISIZE) :: NP,NBP,YFORM,MAXKF,IFORM(PG1),IDC,IDO,ISV(PC),LOGUNT,KDES,   & ! IDC -> IDD; IDD only used in ADVAN10
                               ...
                               MCNTR,KAES,NOPROB,PKPROB,ERPROB,ANRD,NY,SVI(PC),DPI(PM,PG)
!     
...
!
        INTEGER(KIND=ISIZE) :: PNEWIF,NACTIV,ISFINL,A_0FLG,IDXETA(0:PE),MTNOW,       &  ! M -> IDXETA
                               MTPAST(0:PCT),MTNEXT(0:PCT),NEVENT,A_UFLG,IDXETAI(0:PE)            ! N -> NEVENT
!
...
!        
        INTEGER(KIND=ISIZE), PARAMETER :: PD1=PD+1, IREVP=15
!
        REAL(KIND=DPSIZE) :: DDOST(PE),D2DOST(PE,PE),DOSTIM,DOSREC(PD),TSTATE,       &
                             EVTREC(IREVP,PD1),AMNT(PC),DAETA(PC,PE),D2AETA(PC,PE,PE)     ! A -> AMNT
!
...
!
        REAL(KIND=DPSIZE) :: STSTATE(MAXRECID),SA(PC,MAXRECID),SDAETA(PC,PE,MAXRECID), &
                             SD2AETA(PC,PE,PE,MAXRECID),PRED_TIMEVALUE(MAXRECID)
!
...
!
        ...
        INTEGER(KIND=ISIZE) :: PRED_NTIMEVALUES,PRED_TIMETYPE(MAXRECID)
!
...
!
      ...
      USE PRSIZES,      ONLY: ISIZE,DPSIZE,PE,PC,PIR,MAXNRDS,PAST_SIZE,PCMORE
      INTEGER, PARAMETER :: IPAST_OFFSETB=2*(PC*PE+PC-PE-1+PCMORE*((PC-1)*(PE+1)*PE)/2)
      INTEGER, PARAMETER :: LDNLAGS=1000
      REAL(KIND=DPSIZE)   :: PAST( (4*MAXNRDS+2)*PAST_SIZE)
      REAL(KIND=DPSIZE)   :: RPAR(PC+1)
      INTEGER(KIND=ISIZE) :: IPAR(PC+1),IPAST(LDNLAGS+IPAST_OFFSETB),NRDS,NLAG_TIMES,NMTTIME

      REAL(KIND=DPSIZE)   :: LAG_TIMES(10000),MTTIME((4*MAXNRDS+2)*PAST_SIZE)
      DATA NLAG_TIMES /0/
      DATA NMTTIME /0/
      DATA NRDS /0/
      ...
      USE PRSIZES,      ONLY: ISIZE,DPSIZE,PC,PIR,MAXNRDS,PAST_SIZE

!      REAL(KIND=DPSIZE)   :: PAST( (4*MAXNRDS+2)*PAST_SIZE)
      INTEGER(KIND=ISIZE), PARAMETER :: MAXRPAR=MAX(MAXNRDS,PC+1)
      REAL(KIND=DPSIZE), TARGET   :: RPAR(MAXRPAR)
      INTEGER(KIND=ISIZE) :: IPAST(2*PC+10),NLAG_TIMES,NMTTIME
      INTEGER(KIND=ISIZE), TARGET :: IPAR(PC+1)

      REAL(KIND=DPSIZE)   :: LAG_TIMES(10000),MTTIME((4*MAXNRDS+2)*PAST_SIZE)
      DATA NLAG_TIMES /0/
      DATA NMTTIME /0/

      END MODULE PRDDESOLVER

      MODULE PRDDESOLVERE

      USE PRSIZES,      ONLY: ISIZE,DPSIZE,PC,PIR,MAXNRDS,PAST_SIZE,PE

      INTEGER(KIND=ISIZE), PARAMETER :: PASTMAX=PAST_SIZE*(2*MAXNRDS+1)*10
      INTEGER(KIND=ISIZE) :: IPAST(MAXNRDS),NLAG_TIMES,NMTTIME,ICPAST(PC*PE)

      REAL(KIND=DPSIZE)   :: LAG_TIMES(10000),MTTIME((4*MAXNRDS+2)*PAST_SIZE)

!      INTEGER(KIND=ISIZE)  :: PASTM(PAST_SIZE),PASTL(PAST_SIZE),PASTN(PAST_SIZE),PASTNQ(PAST_SIZE)
!      REAL(KIND=DPSIZE) :: TNPAST(PAST_SIZE),HUPAST(PAST_SIZE),HPAST(PAST_SIZE),PASTYH(PAST_SIZE,MAXNRDS,6)

      INTEGER(KIND=ISIZE)  :: NTIMES,MAXEXCEED=0
      REAL(KIND=DPSIZE) :: PAST(PASTMAX)
