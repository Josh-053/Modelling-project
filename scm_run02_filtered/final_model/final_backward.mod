...
IF(BW.LE.65.93) V1BW = ( 1 + THETA(10)*(BW - 65.93))
IF(BW.GT.65.93) V1BW = ( 1 + THETA(11)*(BW - 65.93))
;;; V1BW-DEFINITION END


;;; V1ALB-DEFINITION START
IF(ALB.LE.31.85) V1ALB = ( 1 + THETA(8)*(ALB - 31.85))
IF(ALB.GT.31.85) V1ALB = ( 1 + THETA(9)*(ALB - 31.85))
...
$THETA  (0.001,1.20464,8) ; [1]   CL (L/d)
 (0.001,6.83114,10) ; [2,3] V1  (L)
 (0.01,0.212872,6) ; V2
 (0.001,1.7183,6) ; Q
 ; values are determined in 3 iterations
$THETA  (-1,4.70255,5) ; CLADA1
$THETA  (-10,-0.0843982,0.084) ; CLALB1
 (-0.135,0.177868,10) ; CLALB2
$THETA  (-10,-0.0344875,0.084) ; V1ALB1
 (-0.134,-0.123838,10) ; V1ALB2
$THETA  (-10,0.0160054,0.042) ; V1BW1
 (-0.055,-0.0540409,10) ; V1BW2
...
$OMEGA  0.666155  ;     IIV CL
 0.443187  ;      IIV V
$SIGMA  0.283157  ; EPS(1), proportional
...
ID TIME TAD DV EVID PRED IPRED WRES IWRES RES IRES CWRES NOPRINT ONEHEADER FILE=final_sdtab

$TABLE ; output table for PK parameters
ID CL V1 V2 Q K10 K12 K21 NOPRINT NOAPPEND ONEHEADER FILE=final_patab

$TABLE ; output table for categorical covariates
ID ADA SEX COL NOPRINT NOAPPEND ONEHEADER FILE=final_catab

$TABLE ; output table for continuous covariates
ID BW ALB AGE CREAT NOPRINT NOAPPEND ONEHEADER FILE=final_cotab

$TABLE ; output table for parameters and covariates
ID CL V1 V2 Q K10 K12 K21 ADA SEX COL BW ALB AGE CREAT NOPRINT NOAPPEND ONEHEADER FILE=final_pa_cov
