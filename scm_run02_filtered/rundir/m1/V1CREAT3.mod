...
IF(CREAT.LE.1.03) V1CREAT = ( 1 + THETA(14)*(CREAT - 1.03))
IF(CREAT.GT.1.03) V1CREAT = ( 1 + THETA(15)*(CREAT - 1.03))
;;; V1CREAT-DEFINITION END


;;; V1BW-DEFINITION START
IF(BW.LE.65.93) V1BW = ( 1 + THETA(12)*(BW - 65.93))
IF(BW.GT.65.93) V1BW = ( 1 + THETA(13)*(BW - 65.93))
;;; V1BW-DEFINITION END


;;; V1ALB-DEFINITION START
IF(ALB.LE.31.85) V1ALB = ( 1 + THETA(10)*(ALB - 31.85))
IF(ALB.GT.31.85) V1ALB = ( 1 + THETA(11)*(ALB - 31.85))
;;; V1ALB-DEFINITION END


;;; V1ADA-DEFINITION START
IF(ADA.EQ.0) V1ADA = 1  ; Most common
IF(ADA.EQ.1) V1ADA = ( 1 + THETA(9))
...
IF(ALB.LE.31.88) CLALB = ( 1 + THETA(7)*(ALB - 31.88))
IF(ALB.GT.31.88) CLALB = ( 1 + THETA(8)*(ALB - 31.88))
...
$THETA  (0.001,1.28455,8) ; [1]   CL (L/d)
 (0.001,6.77774,10) ; [2,3] V1  (L)
 (0.01,0.246395,6) ; V2
 (0.001,1.50868,6) ; Q
 ; values are determined in 3 iterations
$THETA  (-1,4.16308,5) ; CLADA1
$THETA  (-0.050,0.022661,0.074) ; CLAGE1
$THETA  (-100000,-0.0771032,0.084) ; CLALB1
 (-0.135,0.166749,100000) ; CLALB2
$THETA  (-1,-0.267183,5) ; V1ADA1
$THETA  (-100000,-0.0118677,0.084) ; V1ALB1
 (-0.134,-0.123039,100000) ; V1ALB2
$THETA  (-100000,0.0180022,0.042) ; V1BW1
 (-0.055,-0.0495879,100000) ; V1BW2
$THETA  (-100000,0.270336,1.190) ; V1CREAT1
 (-1.250,-0.00125,100000) ; V1CREAT2
