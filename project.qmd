...
data <- data %>%
  ...
  group_by(ID) %>%
  mutate(
    # take TIME at dose rows, -Inf otherwise, then cummax to remember last dose time
    last_dose_time = cummax(if_else(EVID == 1, TIME, -Inf)),
    # convert pre-first-dose -Inf to NA
    last_dose_time = if_else(is.finite(last_dose_time), last_dose_time, NA_real_),
