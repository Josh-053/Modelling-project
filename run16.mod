;; 1. Based on: run15
;; 2. Description: PMX001 2CMT TMDD M7+ (FINAL)
;; Joshua I., Ali K.
;; 2025-12-09
...
ID TIME TAD DV EVID PRED IPRED WRES IWRES RES IRES CWRES CRES NOPRINT ONEHEADER FILE=run16_sdtab

$TABLE ; output table for PK parameters
ID CL VMAX KM V1 V2 Q K10 K12 K21 NOPRINT NOAPPEND ONEHEADER FILE=run16_patab

$TABLE ; output table for categorical covariates
ID ADA SEX COL NOPRINT NOAPPEND ONEHEADER FILE=run16_catab

$TABLE ; output table for continuous covariates
ID BW ALB AGE CREAT NOPRINT NOAPPEND ONEHEADER FILE=run16_cotab

$TABLE ; output table for parameters and covariates
ID cAUC CL VMAX KM V1 V2 Q K10 K12 K21 ADA SEX COL BW ALB AGE CREAT NOPRINT NOAPPEND ONEHEADER FILE=run16_pa_cov
