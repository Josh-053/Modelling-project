...
$OMEGA  0.608  ;     IIV CL
 0.375  ;     IIV V1
$SIGMA  0.289  ; EPS(1), proportional
...
$TABLE      ID TIME TAD DV EVID PRED IPRED WRES IWRES RES IRES CWRES
            NOPRINT ONEHEADER FILE=run12_sdtab
 ; output table for PK parameters
$TABLE      ID CL V1 V2 Q K10 K12 K21 NOPRINT NOAPPEND ONEHEADER
            FILE=run12_patab
 ; output table for categorical covariates
$TABLE      ID ADA SEX COL NOPRINT NOAPPEND ONEHEADER FILE=run12_catab
 ; output table for continuous covariates
$TABLE      ID BW ALB AGE CREAT NOPRINT NOAPPEND ONEHEADER
            FILE=run12_cotab
 ; output table for parameters and covariates
$TABLE      ID CL V1 V2 Q K10 K12 K21 ADA SEX COL BW ALB AGE CREAT
            NOPRINT NOAPPEND ONEHEADER FILE=run12_pa_cov
;; REFERENCES
