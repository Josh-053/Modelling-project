...
#Created 2025-12-22 at 15:10

xpose.runno <- '04.mod'
toolname <- 'execute'
pdf.filename <- paste0('PsN_',toolname,'_plots.pdf')
working.directory<-'/home/ali/Documents/GitHub/Modelling-project/modelfit_dir18/'
results.directory <- '..'
subset.variable<-NULL
tab.suffix <- '' 
rscripts.directory <- '/usr/local/share/perl5/5.40/PsN_5_6_0/R-scripts' # This is not used
raw.results.file <- 'raw_results_run04.csv'
theta.labels <- c('VMAX [2]','KM [2]','V1 [5]','V2 [5]','Q  [5]','PROP error ()','ADD (mg/L)')
theta.fixed <- c(FALSE,FALSE,FALSE,FALSE,FALSE,FALSE,FALSE)
omega.labels <- c('IIV VMAX','IIV KM','IIV V1 [5]','IIV V2 [5]','IIV Q  [5]')
omega.fixed <- c(FALSE,FALSE,FALSE,FALSE,FALSE)
sigma.labels <- c('errors already adjusted under $ERROR as THETAs')
sigma.fixed <- c(TRUE)
n.eta <- 5
n.eps <- 1

res.table <- 'run04_cotab'
