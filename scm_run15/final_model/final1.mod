...
$INPUT      DUMMY=DROP ID TIME WEEK DOSE=AMT RATE CONC=DROP ADA
            BW ALB SEX AGE CREAT COL EVID BLQ CONC_M7=DV
...
ID TIME TAD DV cAUC EVID PRED IPRED WRES IWRES RES IRES CWRES CRES NOPRINT ONEHEADER FILE=final1_sdtab

$TABLE ; output table for PK parameters
ID cAUC CL V1 V2 Q K10 K12 K21 NOPRINT NOAPPEND ONEHEADER FILE=final1_patab

$TABLE ; output table for categorical covariates
ID ADA SEX COL NOPRINT NOAPPEND ONEHEADER FILE=final1_catab

$TABLE ; output table for continuous covariates
ID BW ALB AGE CREAT NOPRINT NOAPPEND ONEHEADER FILE=final1_cotab

$TABLE ; output table for parameters and covariates
ID cAUC CL V1 V2 Q K10 K12 K21 ADA SEX COL BW ALB AGE CREAT NOPRINT NOAPPEND ONEHEADER FILE=final1_pa_cov
